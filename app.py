...
if img_file_buffer is not None:
    ...
    index_com = int(np.random.randint(0, 3, size=1))

    class_com = class_names[index_com]

    img_url = ""
    if index_com == 0 : img_url = "images/paper.jpeg"
    elif index_com == 1 : img_url = "images/rock.jpeg"
    elif index_com == 2: img_url = "images/sissors.jpeg"


    win = ""
    if index == 0: #  보
        if index_com == 0 : win="무승부"
        elif index_com == 1 : win="사용자 승"
        elif index_com == 2: win="컴퓨터 승"
    elif index == 1: #바위
        if index_com == 0 : win="컴퓨터 승"
        elif index_com == 1 : win="무승부"
        elif index_com == 2: win="사용자 승"
    elif index == 2: # 가위
        if index_com == 0 : win="사용자 승"
        elif index_com == 1 : win="컴퓨터 승"
        elif index_com == 2: win="무승부"

    # 예측 결과에서 신뢰도를 꺼내 옵니다  
    confidence_score = prediction[0][index]

    img_com = Image.open(img_url)
    st.image(img_com, width=100)

    st.write('사용자:', class_name[2:], '정확도:', confidence_score)
    st.write('컴퓨터:', class_com[2:], end="")
    st.write('결과는 ', win, end="")
